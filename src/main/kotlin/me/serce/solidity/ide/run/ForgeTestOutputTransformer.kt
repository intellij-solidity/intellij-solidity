...
open class ForgetTestOutputTransformer {
  val compilerFailRegex = Regex("^Compiler run failed:$")
  val suiteStartRegex = Regex("^Ran \\d+ tests? for (.*\\.t\\.sol):(.*)$")
  val suiteEndRegex =
    Regex("^Suite result: (\\w+)\\. (\\d+) passed; (\\d+) failed; (\\d+) skipped; finished in (\\d+\\.?\\d+)(.+) \\(.*\\)$")
  val testRegex = Regex("^\\[(PASS|FAIL)(: (.+))?\\] (.*\\(\\)) \\(.* (\\d+)\\)$")
  val summaryRegex =
  ...
  fun transformLine(text: String): List<String>? {
    ...
    testRegex.matchEntire(line)?.let { m ->
      val (passFail, _, error, testName, _) = m.destructured
      activeTest = testName
   ...
   */
  fun escapeTeamCity(s: String): String {
    return s
      .replace("|", "||")
      .replace("'", "|'")
      .replace("\n", "|n")
